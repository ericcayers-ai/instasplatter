//! Optional neural dense-init sidecars.
//!
//! Default policy (v0.3): use the newest license-viable sidecar when present.
//!
...
//! | VGGT-1B (NC) | CC BY-NC | research opt-in only |
...
async fn invoke_launcher(
    ...
    ctx.log(format!("Running {name} dense-init sidecar…"));
    ...
    match path {
        ...
        _ => {
            ctx.log(format!("[{name}] produced no point cloud path"));
            Ok(None)
        }
    }
}

/// Try neural densifiers in priority order. Returns the Gaussian count when
/// one succeeds, or `None` to fall through to COLMAP MVS.
pub async fn try_dense_init(ctx: &JobCtx, images_dir: &Path) -> Result<Option<usize>, String> {
    ...
    let order: Vec<&str> = {
        let mut v = Vec::new();
        // Newest model first when research sidecars are allowed (VGGT-Ω).
    ...
    for name in order {
        ctx.check_cancel()?;
        match invoke_launcher(ctx, name, images_dir).await {
            Ok(Some(ply_path)) => {
                match dense::read_xyzrgb_ply(&ply_path).or_else(|_| {
                    crate::splat::ply::read_ply(&ply_path).map(|c| {
                        let rgb: Vec<[u8; 3]> = c
                            ...
                            .map(|dc| {
                                let to_u8 = |v: f32| {
                                    ((0.5 + crate::splat::SH_C0 * v) * 255.0).clamp(0.0, 255.0)
                                        as u8
                                };
                    ...
                    Err(e) => {
                        ctx.log(format!("[{name}] could not consume output: {e}"));
                    }
                }
            }
            Ok(None) => continue,
